(function themeEngine(){const html=document.documentElement;const skinLink=document.getElementById('skin-css');const toggleBtn=document.getElementById('theme-toggle-btn');const toggleIcon=document.getElementById('theme-toggle-icon');const STORAGE_KEY='palace_theme';function applySkin(isDark){if(!skinLink)return;const href=isDark?skinLink.dataset.dark:skinLink.dataset.light;if(href&&skinLink.getAttribute('href')!==href){skinLink.setAttribute('href',href)}}
function applyIcon(isDark){if(!toggleIcon)return;toggleIcon.classList.toggle('fa-sun',isDark);toggleIcon.classList.toggle('fa-moon',!isDark)}
const initialIsDark=html.classList.contains('dark');applyIcon(initialIsDark);applySkin(initialIsDark);if(toggleBtn){toggleBtn.addEventListener('click',function(e){e.preventDefault();const isDark=html.classList.toggle('dark');localStorage.setItem(STORAGE_KEY,isDark?'dark':'light');applyIcon(isDark);applySkin(isDark)})}
if(!localStorage.getItem(STORAGE_KEY)&&window.matchMedia){window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',function(e){html.classList.toggle('dark',e.matches);applyIcon(e.matches);applySkin(e.matches)})}})();(function scrollReveal(){const targets=document.querySelectorAll('.reveal-on-scroll');if(!targets.length)return;if(!('IntersectionObserver' in window)){targets.forEach(function(el){el.classList.add('is-visible')});return}
